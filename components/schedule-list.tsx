...
export function ScheduleList(
    ...
    return (
        <Accordion type={isSingle as any} defaultValue={defaultValue} className="mx-auto max-w-5xl">
            {schedules?.map((schedule, index) => (
                <AccordionItem value={`item-${index}`} key={index} className="my-1 border-none">
                    <AccordionTrigger className="decoration-transparent rounded-md py-1 px-8 flex justify-start">
                        <div className={cn("text-white text-lg w-full flex justify-start font-bold py-2 px-6 rounded-full", schedule?.day && 'bg-primary-main')}>
                            {schedule?.day}
                        </div>
                    </AccordionTrigger>
