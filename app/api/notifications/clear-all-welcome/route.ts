...
export async function POST(_req: NextRequest) {
  try {
    ...
    // Clear welcome notifications from Redis
    ...
    if (recentNotificationsData && recentNotificationsData.length > 0) {
      ...
      redisCleared = notifications.length - filteredNotifications.length;
    ...
    localStorageMessage =
      "To clear localStorage cache, call the clearWelcomeNotifications function from useNotification hook or clear browser storage manually.";

    return NextResponse.json({
      success: true,
      message: `Cleared ${redisCleared} welcome notifications from Redis`,
      redisCleared,
      localStorageInstructions: localStorageMessage,
    });
  } catch (error) {
